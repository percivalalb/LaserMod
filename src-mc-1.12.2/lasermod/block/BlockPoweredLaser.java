...
public abstract class BlockPoweredLaser extends BlockContainer {
	...
	public BlockPoweredLaser(Material material) {
		super(material);
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
		if(!worldIn.isRemote) {
			if(state.getValue(POWERED) && !this.isLaserSource(worldIn, pos, state))
				worldIn.scheduleUpdate(pos, this, 4);
			else if(!state.getValue(POWERED) && this.isLaserSource(worldIn, pos, state))
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
		if(!world.isRemote)
			if(state.getValue(POWERED) && !this.isLaserSource(world, pos, state))
				cycleState(world, pos, state);
			else if(!state.getValue(POWERED) && this.isLaserSource(world, pos, state))
				cycleState(world, pos, state);
	}
    
    public boolean isLaserSource(World world, BlockPos pos, IBlockState state) {
    	TileEntity te = world.getTileEntity(pos);
    	if(!(te instanceof ILaserReceiver))
    		return false;
    	
    	ILaserReceiver ter = (ILaserReceiver)te;
    	
    	return LaserUtil.isValidSourceOfPowerOnSide(ter, state.getValue(FACING).getOpposite());
    }
