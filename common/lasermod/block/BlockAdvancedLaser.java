...
public class BlockAdvancedLaser extends BlockContainer {
	...
	@Override
	public void breakBlock(World w, int x, int y, int z, int oldBlockId, int oldBlockMeta) {
		TileEntityAdvancedLaser advancedLaser = (TileEntityAdvancedLaser) w.getBlockTileEntity(x, y, z);
		...
		if (advancedLaser != null) {
			for (int j1 = 0; j1 < advancedLaser.upgrades.size(); ++j1) {
				...
				if (itemstack != null) {
					...
					for (float f2 = this.random.nextFloat() * 0.8F + 0.1F; itemstack.stackSize > 0; w.spawnEntityInWorld(entityitem)) {
						...
						entityitem = new EntityItem(w, (double) ((float) x + f), (double) ((float) y + f1), (double) ((float) z + f2), new ItemStack(itemstack.itemID, k1, itemstack.getItemDamage()));
						float f3 = 0.05F;
			...
			w.func_96440_m(x, y, z, oldBlockId);
		}
		super.breakBlock(w, x, y, z, oldBlockId, oldBlockMeta);
	}

	@Override
	public void onBlockPlacedBy(World w, int x, int y, int z, EntityLivingBase par5EntityLiving, ItemStack par6ItemStack) {
		int rotation = determineOrientation(w, x, y, z, par5EntityLiving);
