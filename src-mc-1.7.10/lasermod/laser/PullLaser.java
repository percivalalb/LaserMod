...
public class PullLaser implements ILaser {
	...
	public void performActionOnEntitiesBoth(List<Entity> entities, ForgeDirection dir) {
		for(Entity entity : entities) {
            ...
            if ((entity instanceof EntityFallingBlock) && dir == ForgeDirection.DOWN)
                verticalSpeed = 0.0D;

            if (dir == ForgeDirection.UP && entity.motionY > -maxSpeed)
                entity.motionY += -verticalSpeed;

            if (dir == ForgeDirection.DOWN && entity.motionY < maxSpeed * 0.5D)
                entity.motionY += verticalSpeed;
